...
class GitHubManager:
    ...
    def add_repo(self, url):
        ...
        if not os.path.exists(clone_path):
            ...
            print(f"Repository {repo_name} berhasil di-clone.")
        else:
    ...
    def remove_repo(self, repo_id):
        ...
        for repo in repos:
            if repo[0] == int(repo_id):
                ...
                print(f"Repository {repo[1]} berhasil dihapus.")
                return
        print("ID Repository tidak ditemukan.")

    def update_all_repos(self):
        repos = self.db.fetch_all()
        for repo in repos:
            subprocess.run(["git", "-C", repo[3], "pull"])
            print(f"Repository {repo[1]} diperbarui.")
